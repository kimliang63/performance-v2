...
- 定量行将完成值输入与得分合并为紧凑的“输入 → 得分”结果区，并用五分制进度条承接剩余宽度，消除输入框与得分之间的大面积空白；定性行布局保持不变。
- 本次仅调整既有 V3 员工自评原型交互，不修改 Scope、Spec 或 V3 PRD。
...
## 2026-08-13 绩效分布报表筛选补全

- `current/perf-report-distribution.html`：新增“绩效活动”“等级规则”筛选项，并与统计卡片、等级分布和绩效趋势联动；重置时恢复全部活动、全部等级规则。
- 保留既有导出与导出设置功能；四个筛选控件均补充可访问名称和键盘焦点态。
...
- 得分明细表删除“上级评语”列，仅保留指标评价；参考直属上级评分表单新增独立“评语”卡片，展示员工自评和上级评语，并随周期切换刷新；员工结果页不展示初评等级。
